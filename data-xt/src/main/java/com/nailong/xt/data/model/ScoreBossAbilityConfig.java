package com.nailong.xt.data.model;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonInclude;
import java.util.List;
import java.util.ArrayList;
import java.util.Objects;
import lombok.Builder;
import lombok.EqualsAndHashCode;
import lombok.ToString;

/**
 * 自动生成的配置类。
 * 对应文件: ScoreBossAbility.json
 */
@ToString
@Builder
@EqualsAndHashCode
@JsonInclude(JsonInclude.Include.NON_NULL)
public final class ScoreBossAbilityConfig {

    private final Integer id;
    private final String name;
    private final String desc;
    private final String iconSource;
    private final String value1;
    private final String value2;
    private final List<String> scriptId;
    private final String param1;
    private final String param2;
    private final String param3;
    private final String param4;
    private final String value3;
    private final String value4;
    private final String param5;
    private final String param6;
    private final String param7;
    private final String value5;
    private final String value6;
    private final String value7;

    @JsonCreator
    public ScoreBossAbilityConfig(
        @JsonProperty("Id") Integer id,
        @JsonProperty("Name") String name,
        @JsonProperty("Desc") String desc,
        @JsonProperty("IconSource") String iconSource,
        @JsonProperty("Value1") String value1,
        @JsonProperty("Value2") String value2,
        @JsonProperty("ScriptID") List<String> scriptId,
        @JsonProperty("Param1") String param1,
        @JsonProperty("Param2") String param2,
        @JsonProperty("Param3") String param3,
        @JsonProperty("Param4") String param4,
        @JsonProperty("Value3") String value3,
        @JsonProperty("Value4") String value4,
        @JsonProperty("Param5") String param5,
        @JsonProperty("Param6") String param6,
        @JsonProperty("Param7") String param7,
        @JsonProperty("Value5") String value5,
        @JsonProperty("Value6") String value6,
        @JsonProperty("Value7") String value7
    ) {
        this.id = id;
        this.name = name;
        this.desc = desc;
        this.iconSource = iconSource;
        this.value1 = value1;
        this.value2 = value2;
        this.scriptId = scriptId != null ? 
            new ArrayList<>(scriptId) : 
            new ArrayList<>();
        this.param1 = param1;
        this.param2 = param2;
        this.param3 = param3;
        this.param4 = param4;
        this.value3 = value3;
        this.value4 = value4;
        this.param5 = param5;
        this.param6 = param6;
        this.param7 = param7;
        this.value5 = value5;
        this.value6 = value6;
        this.value7 = value7;
    }

    public Integer getId() {
        return this.id;
    }

    public String getName() {
        return this.name;
    }

    public String getDesc() {
        return this.desc;
    }

    public String getIconSource() {
        return this.iconSource;
    }

    public String getValue1() {
        return this.value1;
    }

    public String getValue2() {
        return this.value2;
    }

    public List<String> getScriptId() {
        return this.scriptId;
    }

    public String getParam1() {
        return this.param1;
    }

    public String getParam2() {
        return this.param2;
    }

    public String getParam3() {
        return this.param3;
    }

    public String getParam4() {
        return this.param4;
    }

    public String getValue3() {
        return this.value3;
    }

    public String getValue4() {
        return this.value4;
    }

    public String getParam5() {
        return this.param5;
    }

    public String getParam6() {
        return this.param6;
    }

    public String getParam7() {
        return this.param7;
    }

    public String getValue5() {
        return this.value5;
    }

    public String getValue6() {
        return this.value6;
    }

    public String getValue7() {
        return this.value7;
    }

}
